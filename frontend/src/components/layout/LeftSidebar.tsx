...
export default function LeftSidebar({ onChangePage, isCollapsed, onClearSelected, user = null }: LeftSidebarProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const [active, setActive] = useState("Home");

  // 🔹 If there is no user, all items will be disabled except “Stations”
  const isLogged = !!user;
  ...
  return (
    ...
    >
      ...
      {/* 🔹 Login/Registration button only if there is NO user */}
      {!isLogged && (
        <div className="mt-4">
          <LeftSidebarButton
            item={loginItem}
